...
mergeResults<- function(x){
	...
	longout<-as.data.frame(melt(results,
					id.vars=colnames(results)[!grepl("^X[.]?\\d{4}\\-?\\d{2}\\-?d{2}\\b",colnames(results))],
					measure.vars=colnames(results)[grepl("^X[.]?\\d{4}\\-?\\d{2}\\-?d{2}\\b",colnames(results))],
					variable.name="date",
					value.name=x,
					variable.factor=F,
					na.rm=T))
...
for(v in uniquevars){mergeResults(v)}
