...
Date<-"6/21/2010"  
time<- "00:06:00"
Date2<-as.POSIXct(paste(Date,time,TimeZone),tryFormat="%m/%d/%Y %H:%M:%S %z")
Time_pastlocalmidnight<-(as.numeric(Date2)-as.numeric(as.POSIXct(paste(Date,"00:00:00",TimeZone),tryFormat="%m/%d/%Y %H:%M:%S %z")))/3600/24
...
HASunrise_deg	=(180/pi)*(acos(cos((pi/180)*(90.833))/(cos((pi/180)*(Latitude))*cos((pi/180)*(SunDeclin_deg)))-tan((pi/180)*(Latitude))*tan((pi/180)*(SunDeclin_deg))))
SolarNoon_LST	=(720-4*Longitude-EqofTime_minutes+(as.numeric(strftime(as.POSIXct(Date2,tz="UTC"),format = '%z'))/100)*60)/1440
...
TrueSolarTime_min	=(((as.numeric(Date2)-as.numeric(as.POSIXct(paste(Date,"00:00:00",TimeZone),tryFormat="%m/%d/%Y %H:%M:%S %z")))/3600/24)*1440+EqofTime_minutes+(4*Longitude)-(60*(as.numeric(strftime(as.POSIXct(Date2,tz="UTC"),format = '%z'))/100)))%%1440

HourAngle_deg	=if(TrueSolarTime_min/4<0) TrueSolarTime_min/4+180 else TrueSolarTime_min/4-180
SolarZenithAngle_deg	=(180/pi)*(acos(sin((pi/180)*(Latitude))*sin((pi/180)*(SunDeclin_deg))+cos((pi/180)*(Latitude))*cos((pi/180)*(SunDeclin_deg))*cos((pi/180)*(HourAngle_deg))))
...
SolarAzimuthAngle_degcwfromN=if(HourAngle_deg>0){
  ((180/pi)*(acos(((sin((pi/180)*(Latitude))*cos((pi/180)*(SolarZenithAngle_deg)))-sin((pi/180)*(SunDeclin_deg)))/(cos((pi/180)*(Latitude))*sin((pi/180)*(SolarZenithAngle_deg)))))+180)%%360
}else{(540-(180/pi)*(acos(((sin((pi/180)*(Latitude))*cos((pi/180)*(SolarZenithAngle_deg)))-sin((pi/180)*(SunDeclin_deg)))/(cos((pi/180)*(Latitude))*sin((pi/180)*(SolarZenithAngle_deg))))))%%360
}
