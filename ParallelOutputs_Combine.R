...
##---- Load Registry
reg<- loadRegistry('S:\\GCMC\\tmp\\BEDDN_Validation_Registry')
...
for(v in as.character(unique(unlist(results[,1])))){
  ...
  longout<- lapply(out2,function(x){as.data.frame(x%>% pivot_longer(cols= colnames(x)[grepl("^.{3}$",colnames(x))],names_to = "theme",values_to = v))})
  
  #out<- do.call(as.data.frame,out)
  #out<-bind_rows(out)
  #out<-do.call("rbind",out)
  #out<- do.call("rbind",results[as.character(unlist(results[,1]))==v,3])
  writeVector(vect(out),paste("BEDDN_Validation_",v,".shp",sep=""),overwrite=TRUE)
  longout<-do.call("rbind",longout)
  write.csv(longout,paste("BEDDN_LONG",v,".csv",sep=""))
  write.csv(out3,paste("BEDDN_",v,".csv",sep=""))
  saveRDS(out3,file=paste("BEDDN_",v,".rds",sep=""))
}
