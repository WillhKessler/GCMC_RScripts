...
##---- v2 Function to 
extract.rastv2= function(vars,period,datchunk,rasterdir,layername,IDfield,Xfield,Yfield,startdatefield,enddatefield,predays=0,weightslayers = NA){
  ...
  if(is.polygons(polygons)){
    ...
    polygons<-project(polygons,crs(rasters))
    output<-terra::extract(x = rasters,y = polygons,ID=F,small=TRUE,bind=T)
    #names(output)<-names(rasters)
...
p.extract.rast <- function(pieces,vars,rasterdir,extractionlayer,layername,IDfield,Xfield,Yfield,startdatefield,enddatefield,predays=0,weightslayers = NA){
  ...
  result<-parallelMap::parallelMap(
...
combine.resultsv2= function(projectname=PROJECT_NAME){
  require('batchtools')
  require('tidyr')
  require('terra')
