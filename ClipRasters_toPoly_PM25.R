...
#########################################
##---Required Inputs---------############
rasterdir<-"S:\\GCMC\\Data\\AirPollution\\PM_25\\Daily"
clippinggeometry<- "S:\\GCMC\\Data\\Massachusetts\\statewide_viewer_fgdb\\MassGIS_Vector_GISDATA.gdb"
clippinglayer<- "OUTLINE_POLY"
outputdirectory<- "S:\\GCMC\\tmp\\PM_25_Daily"
outputname1<- "MA_PM25_1km_daily_"
#########################################
##---- Clipping Boundary
clippingpoly<-vect(readOGR(dsn=clippinggeometry,layer=clippinglayer))

##---- Get unique Years
years<-  unique(sapply(X = strsplit(list.dirs(path = rasterdir,full.names = FALSE,recursive = FALSE),"-"),FUN = function(x){substr(x[11],1,nchar(x[11])-2)})) # Pattern for PM25
...
for(y in years[12:17]){
  ...
  dates<-unique(sapply(X = strsplit(file_path_sans_ext(basename(allRasters)),"_"),FUN = function(x){x[1]})) # Pattern for N02
